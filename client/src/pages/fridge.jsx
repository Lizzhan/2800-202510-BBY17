...
export default function Fridge({ onNavigate }) {
  ...
  useEffect(() => {
    const fetchFridgeData = async () => {
      try {
        const userRes = await fetch('https://two800-202510-bby17.onrender.com/api/auth/me', {
          credentials: 'include',
        });
        const userData = await userRes.json();

        const fridgeRes = await fetch(`https://two800-202510-bby17.onrender.com/api/fridge/${userData.id}`, {
          credentials: 'include',
        });
  ...
  const handleAddToPantry = async (item) => {
    ...
    try {
      ...
      await fetch('https://two800-202510-bby17.onrender.com/api/fridge/add', {
        method: 'POST',
  ...
  const handleSuggestRecipe = async () => {
    ...
    try {
      const res = await fetch('https://two800-202510-bby17.onrender.com/api/match', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
