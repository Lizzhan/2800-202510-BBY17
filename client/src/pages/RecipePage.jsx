...
export default function RecipePage() {
  ...
  useEffect(() => {
    const fetchRecipe = async () => {
      try {
        const res = await fetch(`https://two800-202510-bby17.onrender.com/api/recipes/${id}`, {
          credentials: 'include',
        });
        if (!res.ok) {
  ...
  const handleLikeClick = async () => {
    try {
      if (!liked) {
        ...
        await axios.post(
          'https://two800-202510-bby17.onrender.com/api/save-recipe',
        ...
        await axios.post(
          'https://two800-202510-bby17.onrender.com/api/unsave-recipe',
          { user_recipe_id: recipe.recipe_id },
          { withCredentials: true }
        );
      }
