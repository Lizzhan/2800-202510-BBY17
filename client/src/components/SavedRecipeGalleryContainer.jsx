...
export default function GalleryContainer({ showSavedOnly = false, uploadedOnly = false }) {
  ...
  const fetchUserRecipes = async () => {
    try {
      const response = await axios.get('https://two800-202510-bby17.onrender.com/api/user-recipes', {
  ...
  const handleDeleteRecipe = async (recipeId) => {
    try {
      await axios.delete(`https://two800-202510-bby17.onrender.com/api/user-recipes/${recipeId}`, {
        withCredentials: true
      });
  ...
  useEffect(() => {
    const fetchRecipes = async () => {
      try {
        let data;
        if (showSavedOnly) {
          const response = await axios.get('https://two800-202510-bby17.onrender.com/api/saved-recipes', {
            withCredentials: true
          });
          data = response.data;
          ...
          const response = await axios.get('https://two800-202510-bby17.onrender.com/api/recipes', {
            withCredentials: true
          });
          data = response.data;
        }
