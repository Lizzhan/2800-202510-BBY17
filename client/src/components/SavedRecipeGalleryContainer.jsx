...
export default function GalleryContainer({ showSavedOnly = false, uploadedOnly = false }) {
  ...
  const fetchUserRecipes = async () => {
    try {
      const response = await axios.get('http://ec2-99-79-7-165.ca-central-1.compute.amazonaws.com:3000/api/user-recipes', {
  ...
  const handleDeleteRecipe = async (recipeId) => {
    try {
      await axios.delete(`http://ec2-99-79-7-165.ca-central-1.compute.amazonaws.com:3000/api/user-recipes/${recipeId}`, {
        withCredentials: true
      });
  ...
  useEffect(() => {
    const fetchRecipes = async () => {
      try {
        let data;
        if (showSavedOnly) {
          const response = await axios.get('http://ec2-99-79-7-165.ca-central-1.compute.amazonaws.com:3000/api/saved-recipes', {
            withCredentials: true
          });
          data = response.data;
          ...
          const response = await axios.get('http://ec2-99-79-7-165.ca-central-1.compute.amazonaws.com:3000/api/recipes', {
            withCredentials: true
          });
          data = response.data;
        }
